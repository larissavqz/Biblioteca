...
<html lang="pt-br">
	...
	<body>		
		...
		<center>
			<form id="cadastrar-usuario" style="width: 300px;" method="POST" action="<?= base_url().'usuario/cadastro' ?>">
                ...
                <div class="form-group">
					<label for="senha">CTPS</label>
					<input type="number" class="form-control" id="ctps" name="ctps" placeholder="CTPS">
                </div>
                <div class="form-group">
	...
	<script>
		...
		function validarcadastro()
		{
            ...
            if($(`#ctps`).val()=='')
			{
				alert('CTPS não foi preenchida');
				return false;
            }
            if($(`#cargo`).val()=='')
			{
				alert('Cargo não foi preenchida');
				return false;
			}
			else
			{
